...
## Packaging for a workspace upload

```bash
bash "$CK/scripts/40_package.sh"            # -> coach-k-credit-skill.zip, 53 files
bash "$CK/scripts/40_package.sh" /tmp/x.zip # or to a path you name
```

The uploader caps a `.zip`/`.skill` at **200 files** and reads `name` and `description` out of
the `SKILL.md` frontmatter. Two things follow, and both are enforced by the script rather than
left to memory:

- **Keep `description` a single-line scalar.** A YAML folded block (`description: >`) is valid
  YAML but the uploader's preview reads it as absent, and a skill with no description never
  triggers. `40_package.sh` fails the build if `name` or `description` is missing or spans
  lines.
- **Ship pages as bundles, not as files.** `kb/corpus/*/pages/` is roughly 1,650 files; the
  packaged skill carries one `pages.txt` per document instead, generated by
  `30_bundle_pages.py` and read back by `ckpage.py`. Page text is byte-identical either way.

The package excludes the source PDFs, `kb/_quarantine/`, `search.txt`, `meta.json`,
`links.tsv`, `full.txt` and the extraction/build scripts. None of them are read at answer
time: metadata and links already live in `kb.sqlite3`, and the corpus text lives in the
bundles. Re-run the script after any extraction or index rebuild; it refreshes the bundles
itself so a stale one cannot ship.
